...
@app.route("/")
def home():
    games = len(Games.query.all())
    games_ = db.session.query(Games, Gamelogs).filter(Games.date == Gamelogs.date).filter(Games.homeTeam == Gamelogs.homTeam)
    recent_games = games_.order_by(desc(Games.date)).limit(10)
    recent_added = games_.order_by(desc(Games.id)).limit(10)
    return render_template('main.html', games=games, recent_games=recent_games, recent_added=recent_added)
...
def game_page(team, year, month, day, game_no):
    ...
    url = "https://www.googleapis.com/youtube/v3/videos?part=snippet&id=" + games[0].url + "&key=AIzaSyCLddbBOg4MWCwl5oz29urPC_BSdZe_dYc"
    response = urllib.urlopen(url)
    data = json.loads(response.read())

    channel_name = data["items"][0]["snippet"]["channelTitle"]
    channel_url = data["items"][0]["snippet"]["channelId"]
